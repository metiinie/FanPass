    ...
    <div className="min-h-screen bg-bg flex flex-col md:flex-row font-outfit text-white">
      <aside className="w-full md:w-64 bg-brand-surface border-b md:border-b-0 md:border-r border-white/5 flex flex-col shrink-0 relative z-20">
        <div className="p-8 flex items-center gap-4">
          <div className="bg-white p-2.5 rounded-2xl text-bg shadow-glow-sm">
          ...
          <div>
            ...
            <p className="text-[10px] font-black text-brand-neon uppercase tracking-[0.2em] mt-1">Super Admin</p>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                className={`
                  ...
                  ${isActive 
                    ? "bg-white text-bg shadow-glow-sm" 
                    : "text-gray-400 hover:bg-white/5 hover:text-white"
                  }
                `}
              >
                <Icon className={`w-5 h-5 ${isActive ? "text-bg" : "text-gray-500"}`} />
                {item.name}
              </Link>
            );
          })}
