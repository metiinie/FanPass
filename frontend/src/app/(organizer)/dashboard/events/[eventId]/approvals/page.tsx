...
import { Loader2, CheckCircle2, XCircle, Search, AlertTriangle, ArrowLeft, RefreshCw, ZoomIn, MessageSquare } from "lucide-react";
import Link from "next/link";
import { toast } from "sonner";
import type { SubmissionListItem } from "@/types";

export default function ApprovalsDashboard({ params }: { params: { eventId: string } }) {
  ...
  const fetchSubmissions = async (showSilently = false) => {
    if (!showSilently) setIsLoading(true);
    try {
      const data = await fetchBackend(`/tickets/events/${params.eventId}/submissions?status=${activeTab}`);
      ...
      if (selectedItem) {
        ...
        setSelectedItem(data.submissions[0]);
      }
    } catch (err: any) {
  ...
  useEffect(() => {
    fetchSubmissions();
    // Poll every 10 seconds for new submissions in the needs_review tab
    let interval: NodeJS.Timeout;
    if (activeTab === "needs_review") {
      interval = setInterval(() => fetchSubmissions(true), 10000);
    }
    return () => clearInterval(interval);
  }, [params.eventId, activeTab]);
  ...
  const handleResendSms = async () => {
    ...
    setIsProcessing(true);
    try {
      await fetchBackend(`/tickets/${selectedItem.id}/resend-sms`, { method: "POST" });
    ...
    <div className="h-[calc(100vh-80px)] md:h-full flex flex-col -m-6 md:-m-8">
      ...
      <div className="p-6 border-b border-[#E5E7EB] bg-white flex items-center justify-between shrink-0">
        <div className="flex items-center gap-4">
          <Link href={`/dashboard/events/${params.eventId}`} className="p-2 hover:bg-gray-100 rounded-lg text-gray-500">
            <ArrowLeft className="w-5 h-5" />
          </Link>
          <div>
            <h1 className="text-2xl font-bold font-['Outfit'] text-[#111827]">Approval Queue</h1>
            <p className="text-sm text-[#6B7280]">Review payment receipts and issue tickets</p>
          </div>
        </div>
        <button onClick={() => fetchSubmissions(false)} className="flex items-center gap-2 px-4 py-2 bg-gray-50 hover:bg-gray-100 text-gray-700 rounded-xl text-sm font-medium transition-colors border border-gray-200">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="w-full md:w-96 bg-gray-50 border-r border-[#E5E7EB] flex flex-col shrink-0">
          <div className="p-4 border-b border-[#E5E7EB] bg-white">
            <div className="flex gap-2">
              <button 
                ...
                className={`flex-1 py-2 text-sm font-medium rounded-lg transition-colors ${activeTab === "needs_review" ? "bg-[#1A7A4A] text-white" : "text-gray-600 hover:bg-gray-100"}`}
              >
                Review
              </button>
              <button 
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {isLoading ? (
              <div className="flex justify-center py-8"><Loader2 className="w-6 h-6 animate-spin text-[#1A7A4A]" /></div>
                ...
                <div 
                  ...
                  className={`p-4 rounded-xl cursor-pointer border-2 transition-all ${
                    selectedItem?.id === sub.id 
                      ? "border-[#1A7A4A] bg-white shadow-sm" 
                      : "border-transparent bg-white shadow-sm hover:border-gray-300"
                  } ${["EXTRACTED_LOW_CONFIDENCE", "MANUAL_REVIEW_REQUIRED"].includes(sub.verificationStatus) ? "border-l-4 border-l-orange-500" : ""}`}
                >
                  <div className="flex justify-between items-start mb-2">
                    <p className="font-bold text-[#111827] truncate">{sub.buyerName || sub.buyerPhone}</p>
                    <p className="text-xs text-gray-500 shrink-0">{new Date(sub.issuedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
                  <div className="flex justify-between items-end">
                    ...
                    {["EXTRACTED_LOW_CONFIDENCE", "MANUAL_REVIEW_REQUIRED"].includes(sub.verificationStatus) && (
        ...
        <div className="flex-1 bg-white hidden md:flex flex-col">
          {!selectedItem ? (
            ...
            <>
                ...
                <div className="w-1/2 space-y-6">
                  <div>
                    <h3 className="font-bold text-[#111827] mb-4 flex items-center gap-2">
                      <span className="w-6 h-6 rounded bg-[#1A7A4A]/10 text-[#1A7A4A] flex items-center justify-center text-xs">AI</span>
                      Extracted Data
                    </h3>
                  ...
                  {selectedItem.verificationStatus === "VERIFIED" && (
                    <div className={`p-4 rounded-2xl border ${
                      selectedItem.smsStatus === "SENT" ? "bg-green-50 border-green-100" :
                      selectedItem.smsStatus === "FAILED" ? "bg-red-50 border-red-100" :
                      "bg-gray-50 border-gray-200"
                    }`}>
                      <div className="flex justify-between items-center mb-2">
                        <div className="flex items-center gap-2">
                          <MessageSquare className={`w-4 h-4 ${
                            selectedItem.smsStatus === "SENT" ? "text-green-600" :
                            selectedItem.smsStatus === "FAILED" ? "text-red-600" :
                            "text-gray-400"
                          }`} />
                          <span className="text-sm font-bold text-gray-700">SMS Notification</span>
                        </div>
                        <span className={`text-xs font-bold uppercase ${
                          ...
                          {selectedItem.smsStatus || "NOT SENT"}
                        </span>
                      </div>
                      
                      {selectedItem.smsError && (
                        <p className="text-xs text-red-600 mb-2">{selectedItem.smsError}</p>
                      )}
                      
                      <div className="flex justify-between items-center text-[10px] text-gray-500 uppercase font-bold">
                        <span>Attempts: {selectedItem.smsRetryCount || 0}</span>
                        {selectedItem.lastSmsAttempt && (
                          <span>Last: {new Date(selectedItem.lastSmsAttempt).toLocaleTimeString()}</span>
                        )}
                      </div>
                    </div>
                  )}
              ...
              {/* Action Footer */}
              {["PENDING_EXTRACTION", "EXTRACTED_HIGH_CONFIDENCE", "EXTRACTED_LOW_CONFIDENCE", "MANUAL_REVIEW_REQUIRED"].includes(selectedItem.verificationStatus) ? (
                <div className="p-6 border-t border-[#E5E7EB] bg-gray-50 flex gap-4">
                  ...
                  <button 
                    ...
                    disabled={isProcessing}
                    className="flex-[2] py-3.5 rounded-xl font-bold bg-[#1A7A4A] text-white hover:bg-[#14623b] transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                ...
                <div className="p-6 border-t border-[#E5E7EB] bg-gray-50">
                  <button 
                    ...
                    disabled={isProcessing}
                    className="w-full py-3.5 rounded-xl font-bold bg-[#111827] text-white hover:bg-black transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
                  >
                    {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : <><MessageSquare className="w-5 h-5" /> Resend Ticket SMS</>}
                  </button>
                </div>
              )}
            </>
